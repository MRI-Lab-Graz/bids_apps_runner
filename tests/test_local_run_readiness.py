...
def test_readiness_surfaces_auto_detected_execution_adapter_without_explicit_choice(
    ...
    # Regression coverage for the actual production bug: a fastsurfer_bids
    # container selected, but no explicit execution_adapter set -- this must
    # surface the auto-detected fallback ("fastsurfer-cross") so it's visible
    # before running, instead of only discoverable via a runtime failure.
    bids_dir = tmp_path / "bids"
    bids_dir.mkdir()
    container = tmp_path / "fastsurfer_bids_cuda-v2.5.4.sif"
    container.write_text("fake")
    ...
    by_id = {c["id"]: c for c in data["checks"]}
    assert by_id["execution_adapter"]["detail"].startswith("Will use 'fastsurfer-cross'")
