    ...
    if path:
        ...
        try:
            if cmd == 'docker':
                ...
                result = subprocess.run([cmd, 'version' if cmd != 'datalad' else '--version'], capture_output=True, text=True, timeout=5)
...
def main():
    ...
    if docker_ready or apptainer_ready:
        ...
        print("✗ Docker is installed but NOT RUNNING. Please start Docker (Docker Desktop on macOS/Windows).")
    else:
        print("✗ NO container engine found! You need Docker, Apptainer, or Singularity to run BIDS Apps.")
        
    if results['datalad']:
        print("✓ DataLad is available for automatic data management.")
    else:
        print("! DataLad is NOT available. Automatic retrieval of DataLad datasets will be skipped.")

    # Return non-zero if no container engine found
    if not (results['docker'] or results.get('apptainer') or results.get('singularity')):
        sys.exit(1)
    sys.exit(0)
